var personas = data;
var numberOfRows = personas.length;
var maxNumberOfFields = GetMaxFields();
...
function GetMaxFields()
{
    ...
    InitializePrimaryKey(personas);
    var max = 0;
    for(var i = 0; i<numberOfRows;i++)
    {
        if(Object.keys(personas[i]).length > max)
            max = Object.keys(personas[i]).length;
    }
    return max;
}
...
function CreateTable()
{
	...
	var headerValues = ["ID", "Nombre", "Apellido", "Email", "Género", "IP"];
	var dataParameters = ["id", "first_name", "last_name", "email", "gender", "ip_address"];
	var table = ConstructTable(headerValues, personas.length, maxNumberOfFields, personas, dataParameters);
